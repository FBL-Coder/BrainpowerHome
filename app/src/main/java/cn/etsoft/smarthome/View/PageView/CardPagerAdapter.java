...
public class CardPagerAdapter extends PagerAdapter implements CardAdapter {
    ...
    public void setData(List<String> roomname, List<Object> image) {
        mData = new ArrayList<>();
        mViews = new ArrayList<>();
        mData.addAll(image);
        mName = roomname;
    ...
    public Object instantiateItem(ViewGroup container, final int position) {
        ...
        try {
            bind(mData.get(position), mName.get(position), view);
            CardView cardView = (CardView) view.findViewById(R.id.cardView);
            cardView.setMaxCardElevation(MaxElevationFactor);
            mViews.set(position, cardView);
        }catch (Exception e){
            System.out.print("CardView异常"+e);
        }
        return view;
    }
    ...
    private void bind(Object imgUrl, String name, View view) {
        ...
        if (imgUrl instanceof Integer)
            iv.setImageResource((Integer) imgUrl);
        else
