package cn.etsoft.smarthome.yuyin.recognization;
...
//        sendStatusMessage("临时识别结果，结果是“" + results[0] + "”；原始json：" + recogResult.getOrigalJson());
    ...
    public void onAsrFinalResult(String[] results, RecogResult recogResult) {
        ...
        if (results[0].length() <= 6) {
            MyApplication.mApplication.getSp().play(MyApplication.mApplication.getMusic_dome(), 1, 1, 0, 0, 1);
            return;
        } else if (results[0].length() > 6) {
            if (results[0].contains("打开")
                    || results[0].contains("执行") || results[0].contains("启动") || results[0].contains("运行")
                    || results[0].contains("运行") || results[0].contains("起洞")) {
            } else {
...
//        sendStatusMessage(message + "“；原始json：" + recogResult.getOrigalJson());
        Log.i("最终结果", "onAsrFinalResult: " + message);
